...
#ifdef HEADSUP_FROM_HOME
  pinMode(MB0_Out, OUTPUT);
  pinMode(MB1_Out, OUTPUT);
  digitalWrite(MB0_Out, LOW);
  digitalWrite(MB1_Out, LOW);
#endif
        
  Serial.begin(9600);

#ifndef HEADSUP_FROM_HOME
  ...
  while (isCapturing) {
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (numSamplesRead == NUM_SAMPLES) {
		...
		if (maxValue > 0.8) { 
			if (maxIndex == 0) {
			digitalWrite(MB0_Out, HIGH);          
			}
			else if (maxIndex == 1) {
				digitalWrite(MB1_Out, HIGH);          
			}
		}
#endif
